...
public class ReferenceNozzle extends AbstractNozzle implements ReferenceHeadMountable {
	...
	public void moveTo(Location location, double speed) throws Exception {
		// Shortcut Double.NaN. Sending Double.NaN in a Location is an old API
		// that should no
		// longer be used. It will be removed eventually:
		// https://github.com/openpnp/openpnp/issues/255
		// In the mean time, since Double.NaN would cause a problem for
		// calibration, we shortcut
		// it here by replacing any NaN values with the current value from the
		// driver.
		...
		if (nozzleTip != null && nozzleTip.getCalibration().isCalibrated()) {
			location = location.subtract(nozzleTip.getCalibration().getCalibratedOffset(location.getRotation()));
	...
	public Location getLocation() {
		...
		if (nozzleTip != null && nozzleTip.getCalibration().isCalibrated()) {
			Location offset = nozzleTip.getCalibration().getCalibratedOffset(location.getRotation());
	...
	public Action deleteAction = new AbstractAction("Delete Nozzle") {
		...
		public void actionPerformed(ActionEvent arg0) {
			int ret = JOptionPane.showConfirmDialog(MainFrame.get(),
					"Are you sure you want to delete " + getName() + "?", "Delete " + getName() + "?",
