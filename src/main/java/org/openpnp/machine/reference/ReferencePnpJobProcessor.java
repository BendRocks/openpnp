...
public class ReferencePnpJobProcessor extends AbstractPnpJobProcessor {
	enum State {
		Uninitialized, PreFlight, FiducialCheck, Plan, ChangeNozzleTip, Feed, Pick, Align, Place, Cleanup, Stopped
	}

	enum Message {
		Initialize, Next, Complete, Abort, Skip, Reset
	}

	public static class JobPlacement {
		public enum Status {
			Pending, Processing, Skipped, Complete
		}
		...
		public double getPartHeight() {
			return placement.getPart().getHeight().convertToUnits(LengthUnit.Millimeters).getValue();
		}
	...
	public synchronized boolean next() throws Exception {
		...
		if (fsm.getState() == State.Stopped) {
			/*
			 * If we've reached the Stopped state the process is complete. We
			 * reset the FSM and return false to indicate that we're finished.
			 */
			fsm.send(Message.Reset);
			return false;
		} else if (fsm.getState() == State.Plan && isJobComplete()) {
			/*
			 * If we've reached the Plan state and there are no more placements
			 * to work on the job is complete. We send the Complete Message to
	...
	/*
	 * TODO Due to the Align Skip issue I think we'd be better off replacing
	 * this API with something like List<Message> getOptions(). This would
	 * return a list of options that the caller can take at a given step. Need
	 * to figure out a way to make this generic enough that other JP
	 * implementations can use it, thus it's probably not appropriate to just
	 * use Message, but instead maybe a PnpJobProcessor specific enum. Options
	 * would be things like: * Skip Placement * Try Later * Retry Action *
	 * Continue (Next)
	 * 
	 * Really just need to think about the way a user will want to respond to
	 * various error conditions that arise in each step and see if these can be
	 * generalized in a meaningful way.
	 */
	 ...
	 * 
	 * Check for obvious setup errors in the job: Feeders are available and
	 * enabled, Placements all have valid parts, Parts all have height values
	 * set, Each part has at least one compatible nozzle tip.
	 * 
	 * Populate the jobPlacements list with all the placements that we'll
	 * perform for the entire job.
	 * 
	...
	protected void doPreFlight() throws Exception {
		...
		for (BoardLocation boardLocation : job.getBoardLocations()) {
			...
			for (Placement placement : boardLocation.getBoard().getPlacements()) {
				...
				// Ignore placements that aren't on the side of the board we're
				// processing.
				...
				// Verify that the part height is greater than zero. Catches a
				// common configuration
				// error.
				if (placement.getPart().getHeight().getValue() <= 0D) {
					throw new Exception(
							String.format("Part height for %s must be greater than 0.", placement.getPart().getId()));
				}

				// Make sure there is at least one compatible nozzle tip
				// available
				findNozzleTip(head, placement.getPart());

				// Make sure there is at least one compatible and enabled feeder
				// available
	 ...
	 * 
	 * 1. Create a List<List<JobPlacement>> where each List<JobPlacement> is a
	 * List of JobPlacements that the corresponding (in order) Nozzle can handle
	 * in Nozzle order.
	 * 
	 * In addition, each List<JobPlacement> contains one instance of null which
	 * represents a solution where that Nozzle does not perform a placement.
	 * 
	 * 2. Create the Cartesian product of all of the List<JobPlacement>. The
	 * resulting List<List <JobPlacement>> represents possible solutions for a
	 * single cycle with each JobPlacement corresponding to a Nozzle.
	 * 
	 * 3. Filter out any solutions where the same JobPlacement is represented
	 * more than once. We don't want more than one Nozzle trying to place the
	 * same Placement.
	 * 
	 * 4. Sort the solutions by fewest nulls followed by fewest nozzle changes.
	 * The result is that we prefer solutions that use more nozzles in a cycle
	 * and require fewer nozzle changes.
	 * 
	 * Note: TODO: Originally planned to have this sort by part height but that
	 * went out the window during development. Need to think about how to best
	 * combine the height requirement with the want to fill all nozzles and
	 * perform minimal nozzle changes. Based on IRC discussion, the part height
	 * thing might be a red herring - most machines will have enough Z to place
	 * all parts regardless of height order.
	 */
	protected void doPlan() throws Exception {
		...
		// including
		// one instance of null per Nozzle. The null indicates a possible "no
		// solution"
		// for that Nozzle.
		...
		List<JobPlacement> result = Collect.cartesianProduct(solutions).stream()
				// Filter out any results that contains the same JobPlacement
				// more than once
				.filter(list -> {
					// Note: A previous version of this code just dumped
					// everything into a
					// set and compared the size. This worked for two nozzles
					// since there would
					// never be more than two nulls, but for > 2 nozzles there
					// will always be a
					// solution that has > 2 nulls, which means the size will
					// never match.
					// This version of the code ignores the nulls (since they
					// are valid
					// solutions) and instead only checks for duplicate valid
					// JobPlacements.
					// There is probably a more clever way to do this, but it
					// isn't coming
					// to me at the moment.
				...
				// Sort by the solutions that contain the fewest nulls followed
				// by the
		...
		// Now we have a solution, so apply it to the nozzles and plan the
		// placements.
	...
	/*
	 * TODO: This method is a compromise due to time constraints. Below, there
	 * is doFeed and doPick, which were intended to be used in sequence. I
	 * realized too late that I had made an error in designing the FSM and for
	 * multiple nozzles it was doing feed, feed, pick, pick instead of feed,
	 * pick, feed, pick. The latter is correct while the former is useless.
	 * Since I need to release this feature before Maker Faire I've decided to
	 * just combine the methods to get this done.
	 * 
	 * The whole FSM system needs to be reconsidered. There are two main things
	 * to consider: 1. current FSM cannot handle transitions within action
	 * methods. If it could then we could have doFeed process one
	 * PlannedPlacement, continue to Pick and then have Pick either loop back to
	 * Feed if there are more PlannedPlacements or continue to Align if not. I
	 * don't love this idea because it makes the FSM non-deterministic and thus
	 * harder to reason about.
	 * 
	 * 2. An ideal system would treat each step that required actions for
	 * multiple PlannedPlacements as their own FSM, producing a hierarchy of
	 * FSMs. I've also seen this idea referred to as "fork and join" FSMs and I
	 * have brainstormed this type of system a bit in the image at:
			...
			if (!plannedPlacement.fed) {
				...
				while (true) {
					...
					try {
						...
						if (lastError != null) {
							throw new Exception(String.format("Unable to feed %s. Feeder %s: %s.", part.getId(),
									lastErrorFeeder.getName(), lastError.getMessage()), lastError);
						} else {
							throw new Exception(
									String.format("Unable to feed %s. No enabled feeder found.", part.getId()));
						}
					}
					...
					// Feed the part
					try {
						// Try to feed the part. If it fails, retry the
						// specified number of times
						// before
						// giving up.
						retry(1 + feeder.getRetryCount(), () -> {
							fireTextStatus("Feeding %s from %s for %s.", part.getId(), feeder.getName(),
									placement.getId());
						...
						// If the feed fails, disable the feeder and continue.
						// If there are no
						// more valid feeders the findFeeder() call above will
						// throw and exit the
						// loop.
	...
	protected void doAlign() throws Exception {
		for (PlannedPlacement plannedPlacement : plannedPlacements) {
			...
			PartAlignment.PartAlignmentOffset alignmentOffset = machine.getPartAlignment().findOffsets(part,
					jobPlacement.boardLocation, placement.getLocation(), nozzle);
	...
	protected void doPlace() throws Exception {
		for (PlannedPlacement plannedPlacement : plannedPlacements) {
			...
			// Check if the individual piece has a fiducial check and check to
			// see if the board is enabled
			...
			// Check if there is a fiducial override for the board location and
			// if so, use it.
			...
			Location placementLocation = Utils2D.calculateBoardPlacementLocation(boardLocation,
					placement.getLocation());

			// If there are alignment offsets update the placement location with
			// them
			if (plannedPlacement.alignmentOffsets != null) {

				/*
				 * preRotated means during alignment we have already rotated the
				 * component - this is useful for say an external rotating stage
				 * that the component is placed on, rotated to correct placement
				 * angle, and then picked up again.
				 */
				if (plannedPlacement.alignmentOffsets.getPreRotated()) {
					...
					// Rotate the point 0,0 using the alignment offsets as a
					// center point by the angle
					// that is
					// the difference between the alignment angle and the
					// calculated global
					// placement angle.
					Location location = new Location(LengthUnit.Millimeters).rotateXyCenterPoint(alignmentOffsets,
							placementLocation.getRotation() - alignmentOffsets.getRotation());

					// Set the angle to the difference mentioned above, aligning
					// the part to the
					// same angle as
					// the placement.
					location = location.derive(null, null, null,
					...
					// Add the placement final location to move our local
					// coordinate into global
					// space
					location = location.add(placementLocation);

					// Subtract the alignment offsets to move the part to the
					// final location,
					// instead of
					// the nozzle.
	...
	/**
	 * Discard the picked part, if any. Remove the currently processing
	 * PlannedPlacement from the list and mark the JobPlacement as Skipped.
	 * 
	 * @throws Exception
	 */
	...
	// Sort a List<JobPlacement> by the number of nulls it contains in ascending
	// order.
	Comparator<List<JobPlacement>> byFewestNulls = (a, b) -> {
		return Collections.frequency(a, null) - Collections.frequency(b, null);
	};

	// Sort a List<JobPlacement> by the number of nozzle changes it will require
	// in
	// descending order.
